...
df = pd.read_csv('slave-ships/full-dataset-percentages.csv')
...
kept_columns = [year, embarked, disembarked, children]
...
# removes blank rows
# df = df.dropna()
df[children] = df[children].interpolate(method='linear', limit_direction='both')
df = df.dropna(subset=[embarked, disembarked, children])
...
# loop through dataframe, see if theres a value if not just ignore it but if there is do this:
...
# saves as csv
df.to_csv('slave-ships/data/cleaned-children5.csv', index=False)
